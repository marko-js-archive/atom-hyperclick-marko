'use strict';
let defaultMarkoCompiler = require('marko/compiler');
let path = require('path');
let fs = require('fs');
let lassoPackageRoot = require('lasso-package-root');
...
function getTaglibLookup(textEditor) {
    ...
    if (filePath) {
        ...
        markoCompiler = loadMarkoCompiler(dir);
    } else {
        dir = getDefaultDir();
...
module.exports = {
    activate: function() {
        ...
        return Promise.resolve()
            .then(() => {
                return require('atom-package-deps').install('hyperclick-marko');
            })
            .catch((e) => {
                console.log(e);
            });
